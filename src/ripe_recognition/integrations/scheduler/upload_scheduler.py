...
def _upload_today_errors(settings: Settings) -> None:
    if not settings.destination_upload:
        print("[UPLOAD] DESTINATION_UPLOAD tidak diset, skip.")
        return
    ...
    for folder_type, source_dir in [
        ...
        if not source_dir.exists():
            print(f"[UPLOAD] No source folder for today: {source_dir}")
            continue

        dest_path = _get_unique_path(destination / f"{folder_type}_{today}")
        try:
            shutil.copytree(str(source_dir), str(dest_path))
            print(f"[UPLOAD] Copied {source_dir} -> {dest_path}")
            shutil.rmtree(str(source_dir))
            print(f"[UPLOAD] Deleted {source_dir}")
        except Exception as e:
            print(f"[UPLOAD] Failed to upload {source_dir}: {e}")
...
class UploadScheduler:
    ...
    def start(self) -> None:
        ...
        print(f"[SCHEDULER] Upload scheduled at {self.settings.upload_hour:02d}:{self.settings.upload_minute:02d}")
