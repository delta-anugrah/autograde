...
| `make demo-off` | hapus data demo (sepuluh plat `PLATES`) dan **berhenti** di situ — beda dari `AKSI=reset` yang langsung mengisi ulang. Data sungguhan tidak disentuh (`wipe()` menyaring per plat). Jalankan sesudah showcase, **sebelum** uji coba sungguhan: janjang demo berstempel sampai mendekati jam sekarang, jadi selama masih ada dia menutupi baris yang baru digrading |
