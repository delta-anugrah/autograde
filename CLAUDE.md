...
| POST | `/internal/hapus-data` | ← dari konsol (Danger Zone): tulis penanda `state/.hapus-data` lalu keluar; data line dihapus **saat boot berikutnya**, sebelum store mana pun membuka berkasnya. **409** kalau line sedang dipasangi truk. Router `routes/internal_bahaya.py` — **tanpa torch**, jadi teruji di CI |
...
| POST | `/api/console/dev/bahaya/hapus-data` | `{mode:"transaksi"\|"semua", konfirmasi:"HAPUS"}`. **400** konfirmasi/mode salah, **409** `bahaya_ditolak` (+`params.hambatan`) — keduanya tidak mengubah apa pun. Lihat aturan 25 |
...
25. **Danger Zone: line menghapus datanya SENDIRI, saat BOOT; setelan & lisensi selamat** (2026-09-25).
    ...
    konsol. Line menulis penanda `state/.hapus-data`, keluar (`os._exit`), dan **awal
    lifespan `main.py`** menghapus isi `artifacts/` + `state/` SEBELUM store mana pun
    membuka berkasnya (SQLite yang sedang dibuka tidak boleh dihapus dari bawah
    prosesnya). Penanda dihapus **paling akhir** dan hanya kalau semuanya berhasil — boot
    yang terputus mengulang, bukan meninggalkan separuh data.
    ⚠️ **Konsol MENUNGGU line mati** (`/health`, maks 5 dtk) sebelum mengosongkan datanya
    sendiri: line keluar 1 detik sesudah menjawab, dan janjang yang lewat di detik itu
    masih dikirim ke konsol — tanpa menunggu, baris grading yang fotonya sudah hilang
    tertinggal.
    ...
    **Hambatan (409)**: line mati, truk terpasang, outbox line belum kosong (tak terbaca =
    belum kosong), antrean AutoERP `pending` kalau `ERP_URL` terisi, dan — mode semua —
    tidak ada hash akun bawaan DAN tidak ada AutoERP (konsol akan terkunci). Line juga
